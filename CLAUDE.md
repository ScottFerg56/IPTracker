...
- `NetworkDevice` — plain data model with 5 string properties (`Name`, `IpAddress`, `Manufacturer`, `MacAddress`, `Comments`). `LoadFromXml(path)` parses `<row>` elements with matching attributes; `SaveToXml(devices, path)` writes them back in the same format.
